...
SRC_URI  = "git://cobalt.googlesource.com/cobalt.git;protocol=https;name=cobalt;branch=20.lts.stable"
SRC_URI += "git://cobalt.googlesource.com/depot_tools.git;protocol=https;destsuffix=depot_tools;name=depottools"
SRC_URI += "${CMF_GIT_ROOT}/rdk/components/generic/cobalt;protocol=${CMF_GIT_PROTOCOL};destsuffix=starboard;name=starboard;branch=20.lts.stable"

SRCREV_cobalt = "20.lts.5"
SRCREV_depottools = "913305037df7027dc118253b7c2d3655d181c612"
# Oct 21 2020
SRCREV_starboard = "67be4cfa5ccab4385dcba0985ae6127b8ccdd335"
...
do_configure() {
    ...
    ${S}/src/cobalt/build/gyp_cobalt -C gold -C devel ${COBALT_PLATFORM}
}
...
do_compile_append() {
    if ${@bb.utils.contains('PACKAGECONFIG', 'nplb', 'true', 'false', d)}; then
        ...
        ninja -C ${S}/src/out/${COBALT_PLATFORM}_devel -j1 nplb
    fi
}
do_install_append() {
    if ${@bb.utils.contains('PACKAGECONFIG', 'nplb', 'true', 'false', d)}; then
